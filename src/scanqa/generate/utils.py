...
def interpolate_point_cloud(points, k=2):
    ...
    new_points = np.concatenate([new_points, new_attrs], axis=2).reshape(
        -1, points.shape[1]
    )
...
def filter_point_cloud(point_cloud, angle_threshold_degrees=90, fix=False):
    ...
    normalized_xyz = (xyz - center) / np.linalg.norm(xyz - center, axis=1)[
        :, np.newaxis
    ]
...
def get_xml(
    ...
    xml_object_segment = (
        xml_ball_segment if object_type == "point" else xml_cube_segment
    )
...
def get_mllm_messages(prompt, base64_image=None, view_mode="single_view"):
    query = prompt

    if view_mode == "multi_view":
        if "where" in query.lower():
            query = prompt_where_multiview + query
        if "how many" in query.lower():
            query = prompt_number_multiview + query
        if "what color" in query.lower() or "what is the color" in query.lower():
            query = prompt_color_multiview + query
        elif "what shape" in query.lower():
            query = prompt_shape_multiview + query
        elif "what type" in query.lower():
            query = prompt_type_multiview + query
        elif "what kind" in query.lower():
            query = prompt_kind_multiview + query
        elif "what is" in query.lower():
            query = prompt_what_multiview + query
        else:
            query = prompt_other_multiview + query
    else:
        if "where" in query.lower():
            query = prompt_where_single_view + query
        if "how many" in query.lower():
            query = prompt_number_single_view + query
        if "what color" in query.lower() or "what is the color" in query.lower():
            query = prompt_color_single_view + query
        elif "what shape" in query.lower():
            query = prompt_shape_single_view + query
        elif "what type" in query.lower():
            query = prompt_type_single_view + query
        elif "what kind" in query.lower():
            query = prompt_kind_single_view + query
        elif "what is" in query.lower():
            query = prompt_what_single_view + query
        else:
            query = prompt_other_single_view + query
